...
<!-- 7. third issue -->
Lastly, concerning the third limitation, although separating trait estimation from follow-up analysis can simplify the latter, focusing solely on mean trait estimates or residuals obtained from the estimation stage inadvertently treats these outputs as fixed values rather than as uncertain parameter estimates [@McElreath_2020; @Rivera_et_al_2025]. This separation thus prevents the propagation of uncertainty from the estimation stage to the follow-up analyses [@Pritikin_2020], which can bias parameter estimates and reduce their precision. The direction and magnitude of these biases are often difficult to predict. Effects may be attenuated, inflated, or remain unchanged depending on the level of uncertainty in the scores and the true effects under investigation [@McElreath_2020; @Kline_et_al_2023; @Hoyle_et_al_2023]. In addition, reduced precision lowers statistical power and increases the risk of Type I and Type II errors [@McElreath_2020].
...
<!-- Sven and Tine 2026.03.09, time: 00:49:40 - 00:50:05
Tine statement: There are some really valuable attempts to incorporate certain features of the data, but one overarching framework that allows to consider everything is currently unavailable. By building on the work of ... -->
Notably, the ITCJ analysis directly addresses all of these limitations by providing a unified and systematic approach for analyzing CJ data [@Rivera_et_al_2025]. Specifically, the ITCJ analysis accommodates the complex data-generating structures commonly observed in applied CJ settings, allowing researchers to model the processes of substantive interest while propagating uncertainty across components, rather than relying on restricted model specifications.
