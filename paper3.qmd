...
To address its two research objectives, this study follows the *Bayesian research workflow* [@Depaoli_et_al_2017; @Pearl_et_al_2018; @Neal_2020; @Gelman_et_al_2020; @Schad_et_al_2020; @Betancourt_2020; @McElreath_2024b; @McElreath_2024c], as depicted in @fig-worflow. The workflow consists of iterative stages progressing from theoretical assumptions through parameter estimation and data prediction, while supporting the study's dual focus on *empirical validation* and tutorial-style *implementation*. Crucially, given the emphasis on empirical validation, this study employs a synthetic dataset derived from a CJ data-generating process grounded in the speech-quality literature.

![Bayesian research workflow.](3_results/theory/figures/png/workflow.png){#fig-worflow fig-align="center" fig-pos="H" width=100%}

The workflow begins by translating the theoretical understanding of the synthetic CJ system into formal *estimands*--the targets of inference [@Everitt_et_al_2010]. Specifically, drawing on the relevant literature, the study makes explicit the assumptions underlying the CJ structure of interest using a DAG and SCM. It then identifies and formally defines the estimands as target parameters. 

The workflow then proceeds from theory through the design phase to the data-generation phase. First, it simulates a synthetic conceptual population that reflects the specified theoretical assumptions. From this population, it draws a synthetic sample and corresponding comparison datasets (the latter constituting the *analysis dataset*).

Next, building on the theoretical assumptions formalized as a DAG and an SCM, the ITCJ analysis specifies a probabilistic model. From this formulation, the approach derives one or more statistical models (*estimators*) tailored to the CJ system under study. For the CCJ analysis, the study adopts the BTL model [@Bradley_et_al_1952; @Luce_1959] as the estimator, given its extensive use in the CJ literature. From this model, it extracts trait estimates and residuals, which are then used in subsequent follow-up analyses with appropriate models (see @sec-methods_step5_1). In this phase, the study conducts prior predictive checks to establish meaningful priors and to assess their implications within the model context before incorporating any empirical information [@McElreath_2020].

After specifying the estimators and their corresponding priors, the study fits the statistical models to obtain *estimates* of the target parameters under both approaches. Crucially, it evaluates these estimates using diagnostic and post-predictive assessments, including checks for stationarity, convergence, and mixing, as well as assessments of parameter recovery and predictive performance.
