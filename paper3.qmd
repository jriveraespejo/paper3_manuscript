...
Pairwise comparison data can be analyzed using two main approaches: the CCJ and the ITCJ analysis. This section provides a detailed description of both.
...
In practice, the CCJ analysis proceeds through a sequence of distinct analytical steps, each serving a distinct purpose [@Pollitt_2012a; @Pollitt_2012b; @Jones_et_al_2019; @Boonen_et_al_2020; @Chambers_et_al_2022; @Bouwer_et_al_2023]. The approach typically includes one or more of the following steps. First, researchers fit narrowly specified Thurstone-based models to the CJ data, yielding two primary outputs: (1) point estimates of stimulus latent scores with their associated standard errors, and (2) residuals at the comparison level. These outputs serve as the foundation for any follow-up analyses.

<!-- 2. Other steps in the process -->
<!-- Sven and Tine p2 2026.02.02, time: 00:09:55 - 00:11:05 -->
Second, researchers summarize or fit models to the estimated mean latent scores. This step serves multiple purposes, including aggregating stimulus-level point estimates to the individual level, partitioning variability between and within individuals, and drawing inferences about factors that influence trait scores. For example, @Maydeu-Olivares_et_al_2005 fitted multiple factor models to latent means that represent individuals' preferences for compact cars, aiming to provide a more parsimonious representation of these preferences. Similarly, @Boonen_et_al_2020 applied a multilevel regression model to stimulus mean latent scores to examine whether children's age or hearing status affects their intelligibility scores.

A third step may involve summarizing or modeling the residuals. Researchers typically use this step to aggregate the remaining variability at the judge level, partition residual variability between and within judges, test for systematic biases, and identify potential misfitting judgments, stimuli, or judges. For instance, @Wu_2025 fitted an analysis of variance (ANOVA) model to judges' infit statistic to examine whether judge expertise influences judgment behavior. The infit statistics--a weighted average of the squared Pearson residuals--potentially indicate the extent to which judges' decisions deviate from expectations relative to other judges [@Wright_et_al_1982]. 

<!-- 3. problems with the stepwise approach -->
The CCJ analysis has become standard practice in education, linguistics, psychology, and even criminology [see e.g., @Wu_2025; @Thwaites_et_al_2024; @Maydeu-Olivares_et_al_2005; @Seymour_et_al_2025]. Nevertheless, despite its widespread adoption, this stepwise approach has two important limitations: it treats outputs from earlier steps as fixed quantities, and it imposes restrictive models at each step.

Regarding the first limitation, the CCJ analysis treats outputs from earlier stages as fixed quantities rather than as uncertain parameter estimates, thereby limiting the propagation of uncertainty from one step to the next [@Pritikin_2020]. When researchers ignore this uncertainty, estimates may become biased and less precise. The direction and magnitude of these biases can be unpredictable: effects may be attenuated, inflated, or remain unaffected depending on the degree of uncertainty in the scores and the true effects under investigation [@McElreath_2020; @Kline_et_al_2023; @Hoyle_et_al_2023]. Moreover, reduced precision lowers statistical power and increases the risk of Type I or Type II errors [@McElreath_2020].
...
Regarding the second limitation, researchers often impose restrictive models at each stage of the CCJ analysis, which limit their ability to capture key features of the data-generating process. Although recent work incorporates selected features, most studies propose new Thurstone-based models that address only one or two at a time. As a result, important data components remain unrepresented, leading to model misspecification and potentially biased and imprecise parameter estimates [@Everitt_et_al_2010].

For example, @Seymour_et_al_2025 extended the BTL model to include tied outcomes and spatially correlated stimuli. They applied the model to an honour-based abuse dataset. Despite the model sophistication, the authors did not assess potential bias arising from the convenience sample, including judge biases and judge-specific effects. Moreover, because the analysis was limited to a single ward within one county, they did not account for ward-level effects or hierarchical clustering structures, such as wards nested within counties. This oversight restricts the generalizability of the findings. Notably, similar limitations are evident in @Pritikin_2020, @Schramm_et_al_2021, @Seymour_et_al_2022, and @Gray_et_al_2024, where models omit relevant sources of heterogeneity or design features that may affect parameter estimates, and ultimately, inferences.
...
Notably, the ITCJ analysis directly addresses these two limitations by providing a unified and systematic framework for analyzing CJ data [@Rivera_et_al_2025]. Specifically, it allows researchers to model the processes of substantive interest while propagating uncertainty across components, rather than relying on restrictive statistical model specifications.
