...
To address its two research objectives, this study follows the *Bayesian research workflow* [@Depaoli_et_al_2017; @Pearl_et_al_2018; @Neal_2020; @Gelman_et_al_2020; @Schad_et_al_2020; @Betancourt_2020; @McElreath_2024b; @McElreath_2024c], as depicted in @fig-worflow. The workflow consists of iterative stages progressing from theoretical assumptions through parameter estimation and data prediction, thereby supporting the study's dual focus on empirical model validation and tutorial-style implementation. Crucially, given the emphasis on validation, the study employs a synthetic dataset derived from a CJ data-generating process grounded in the speech-quality literature, enabling model validation under controlled conditions.
...
The data-generating assumptions of this study draw on key features and findings from @Boonen_et_al_2020. In particular, the study also examines whether children's age and hearing status influence overall speech quality, assuming a CJ task with the same core design features and empirical findings reported by the aforementioned authors.
...
@fig-CP_dag presents a DAG with the incorporated design features. The DAG represents the "true" data-generating process underlying the synthetic datasets. It depicts stimuli nested within individuals who differ in characteristics $X_{I}$, such as age and hearing status. It also includes judges who vary in speech-judgment expertise $Z_{J}$ and who provide a single judgment per stimulus pair, reflected in the absence of variables $Z_{JK}$ and $e_{JK}$ previously shown in @fig-ITCJ_dag.

Finally, similar to @fig-ITCJ_dag, the DAG represents the sampling and comparison mechanisms ($S$ and $C$, respectively) without incoming arrows, indicating their independence from all other variables in the DAG. This structure implies that the study samples both stimuli and individuals using SRSg algorithms [@Lawson_2015] and assigns judges to comparisons using random allocation comparative designs [@Bramley_2015] to generate the synthetic sampling and comparison datasets, respectively.
...
Next, the study incorporates Boonen and colleagues' empirical findings through a statistical model derived from the probabilistic model implied by the system's SCM, as depicted in @fig-CP. The model conditions on the individual-level variable vector $X_{I} = \{ X_{Ic}, X_{Id}[g=1], X_{Id}[g=2], X_{Id}[g=3] \}$ with corresponding effect vector $\beta_{XI} = \{ \beta_{XIc}, \beta_{XId[g=1]}, \beta_{XId[g=2]}, \beta_{XId[g=3]} \}$, as well as the judge-level variable vector $Z_{J} = \{ Z_{Jd}[g=1], Z_{Jd}[g=2], Z_{Jd}[g=3] \}$ with corresponding effect vector $\beta_{ZJ} = \{ \beta_{ZJd[g=1]}, \beta_{ZJd[g=2]}, \beta_{ZJd[g=3]} \}$.

Specifically, the study assumes that each one-unit increase in age $X_{Ic}$ leads to a $0.1$-logit increase in speech quality for children aged five to nine, as captured by $\beta_{XIc} = 0.1$. In addition, the model assumes a strict ordering in hearing-status effects, such that normal-hearing (NH) children ($X_{Id}[g=1]$) exhibit higher speech quality than hearing-impaired children with hearing aids (HI-HA, $X_{Id}[g=2]$), who in turn outperform hearing-impaired children with cochlear implants (HI-CI, $X_{Id}[g=3]$). This ordering is reflected in the parameters $\beta_{XId[g=1]} = 1$, $\beta_{XId[g=2]} = 0$, and $\beta_{XId[g=3]} = -1$. 

Moreover, the study assumes that judges do not introduce systematic bias on average: audiologists (AU, $Z_{Jd}[g=1]$), primary teachers (PT, $Z_{Jd}[g=2]$), and inexperienced listeners (IL, $Z_{Jd}[g=3]$) all have zero mean effects, such that $\beta_{ZJd[g=1]} = \beta_{ZJd[g=2]} = \beta_{ZJd[g=3]} = 0$.

Furthermore, consistent with @Boonen_et_al_2020, the study assumes that trait variability differs across hearing-status groups. In particular, NH children exhibit higher variability ($s_{XId[g=1]} = 1.5$) than HI-HA and HI-CI children, both of whom are assumed to show lower and equal variability $(s_{XId[g=2]} = s_{XId[g=3]} = 0.75)$.

In addition, the study assumes greater unexplained variability between individuals than within individuals, i.e., the stimulus level. Formally, the model specifies $s_I = \big(s_{XId[g=1]} + s_{XId[g=2]} + s_{XId[g=3]}\big)/3 = 1$, which exceeds stimulus-level variability $s_A = 0.2$.

[TO DO]{style="color:red;"}

<!-- Notably, although the original study found no evidence of systematic judge bias, this study does assume ; accordingly, the present simulation treats the absence of judge bias as a modeling assumption. Finally, while judges’ experience levels did not explain differences in mean latent trait levels between stimuli, the original study did not assess whether experience influenced variability in those traits. -->
...
## From Theory to Estimands: Data-generating assumptions {#sec-methods_step2}
