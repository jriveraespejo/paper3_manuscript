...
Several studies have made valuable attempts to incorporate specific features that address some of the limitations discussed above. However, most propose new Thurstone-based models that address only one or two issues at a time. As a result, other relevant components of the CJ process remain unrepresented, which may lead to model misspecification and produce biased or imprecise parameter estimates [@Everitt_et_al_2010]. For instance, @Seymour_et_al_2025 extended the BTL model to incorporate spatially correlated stimuli and tied outcomes, applying it to measure honour-based abuse. While these features capture important aspects of their CJ data, additional relevant factors were not included, such as potential bias from the convenience sample, judge-specific effects, or hierarchical structures like wards nested within counties. Similar patterns appear in @Pritikin_2020, @Schramm_et_al_2021, @Seymour_et_al_2022, and @Gray_et_al_2024, where models typically address one or two key design features or sources of heterogeneity at a time. 
