...
In contrast to the CCJ approach, the ITCJ analysis does not begin with a predefined statistical model applied to the CJ data, from which primary outputs are extracted for subsequent analysis. Instead, it starts by explicitly representing the relationships among observed and latent variables that are thought to characterize the data-generating process of the CJ system under study. This structure is represented using a *Directed Acyclic Graph (DAG)* and an associated *Structural Causal Model (SCM)* [@Morgan_et_al_2014; @Gross_et_al_2018; @Neal_2020]. 

On the basis of this causal representation, the approach then derives one or more *probabilistic* and *statistical* models tailored to the system under study. These models are subsequently used to estimate, summarize, or conduct statistical inference on the traits of interest. @fig-ITCJ shows a DAG and SCM representing a plausible data-generating process for a CJ-based speech quality study.
...
::: {#fig-ITCJ layout="[90,10]"}

![ITCJ analysis, DAG](3_results/theory/figures/png/CJ_TM_15.png){#fig-ITCJ_dag fig-align="center" fig-pos="H" width=100%}
...
Plausible data-generating process for a CJ-based speech quality study
:::
...
In this representation, observed variables are depicted as closed circles, while latent (unobserved) variables are depicted as open circles. Building on this structure, $X_{I}$ denotes the set of variables believed to influence the individual traits $T_{I}$, i.e., the individuals' ability to produce high-quality speech. Examples of such individual-level variables include hearing status and age, the latter serving as a proxy for language development [@Boonen_et_al_2020]. Similarly, $X_{IA}$ denotes the set of variables assumed to influence the stimulus traits $T_{IA}$, which represent the latent quality of the stimulus under comparison. Examples of such stimulus-level variables include the length of the speech sample and whether the sample originates from spontaneous speech or imitation. Notably, these stimulus traits correspond to what the CJ literature refers to as *discriminal processes*, which are though to capture the psychological effect a stimulus exerts on the judge, or more simply, the judge's perception of the stimulus quality [@Thurstone_1927a; @Thurstone_1927b].
...
With the DAG and SCM characterizing the data-generating process of the CJ system under study, researchers can translate these representations into a probabilistic model that factorizes the joint distribution of the system's variables into a product of simpler *conditional probability distributions (CPDs)* [@Pearl_et_al_2016; @Neal_2020], as illustrated in @fig-ITCJ_prob. Finally, one or more statistical models tailored to this CJ system can be derived from this formulation [see @sec-methods_step5_2].

At this stage, it becomes clear that the ITCJ approach differs fundamentally from the CCJ approach in its treatment of parameter estimation and inference, ultimately addressing the three main limitations of the CCJ analysis. First, although the ITCJ still relies on Thurstone-based models for trait estimation, these models are no longer treated as restrictive simplifying abstractions; instead, they are adapted to the specific data-generating process of the CJ system under study, allowing researchers to explicitly model the processes of substantive interest. Second, the approach explicitly incorporates relevant features of CJ assessment design, including the sampling and comparison mechanisms as well as judge biases. Third, the ITCJ no longer separates trait estimation from follow-up analyses, but rather embeds the full analysis workflow within a unified and systematic framework [@Rivera_et_al_2025]. This integration enables the propagation of uncertainty across all levels of the assumed hierarchical structure of the data. 
