...
In contrast to the CCJ approach, the ITCJ analysis does not begin with a predefined statistical model applied to the CJ data, from which primary outputs are extracted for subsequent analysis. Instead, it starts by explicitly representing the relationships among observed and latent variables that are thought to characterize the data-generating process of the CJ system under study. This structure is represented using a *Directed Acyclic Graph (DAG)* and an associated *Structural Causal Model (SCM)* [@Morgan_et_al_2014; @Gross_et_al_2018; @Neal_2020]. On the basis of this causal representation, a *probabilistic* model is derived, followed by one or more *statistical* models tailored to the system under study. These models are then used to estimate, summarize, or conduct statistical inference on the traits of interest. 
...
Once the SCM [-@fig-ITCJ_scm] characterizing the assumed data-generating process of the CJ system is specified, researchers can translate it into a probabilistic model that factorizes the joint distribution of the variables into a product of simpler *conditional probability distributions (CPDs)* [@Pearl_et_al_2016; @Neal_2020], as illustrated in @fig-ITCJ_prob. From this formulation, one or more statistical models tailored to the CJ system can then be derived (see @sec-methods_step5_2).

At this stage, it becomes clear that the ITCJ approach differs fundamentally from the CCJ approach in its treatment of trait estimation, summaries, and inference. First, although the ITCJ still relies on Thurstone-based models for trait estimation, these models are no longer treated as restrictive simplifying abstractions; instead, they are adapted to the specific data-generating process of the CJ system under study, allowing researchers to explicitly model the processes of substantive interest. Second, the approach explicitly incorporates relevant features of CJ assessment design, including the sampling and comparison mechanisms as well as judge and judgment biases. Third, the ITCJ no longer separates trait estimation from follow-up analyses, but rather integrates both within a unified and systematic approach [@Rivera_et_al_2025]. This integration enables the propagation of uncertainty across all levels of the assumed hierarchical structure of the data. In this way, the ITCJ approach addresses the three main limitations of CCJ analysis.
...
To address its two research objectives, this study follows the *Bayesian research workflow* [@Depaoli_et_al_2017; @Pearl_et_al_2018; @Neal_2020; @Gelman_et_al_2020; @Schad_et_al_2020; @Betancourt_2020; @McElreath_2024b; @McElreath_2024c], as depicted in @fig-worflow. The workflow consists of iterative stages that move from theoretical assumptions to empirical estimation, while supporting the study's dual focus on *empirical validation of the ITCJ approach* and its tutorial-style *implementation*.

![Bayesian research workflow.](3_results/theory/figures/png/workflow.png){#fig-worflow fig-align="center" fig-pos="H" width=100%}

<!-- The workflow begins with translating theoretical understanding into formal estimands—the specific quantities we aim to estimate. Next, we design a data-generating process that reflects our theoretical assumptions and implement it through simulation [@gelman2006; @kruschke2014; @mcelreath2020]. We then develop statistical models (estimators) appropriate for our estimands and design, specify prior distributions that encode our knowledge before observing data, and evaluate these priors through prior predictive checks. -->

<!-- After establishing appropriate priors, we fit models to simulated data and conduct comprehensive diagnostics to assess stationarity, convergence, and mixing of Markov chains [@vehtari2021]. We evaluate parameter recovery to determine whether models can accurately estimate known parameter values, assess in-sample predictive accuracy, and estimate approximate out-of-sample fit. This evaluation informs iterative model refinement until we obtain sufficiently trustworthy results. -->


<!-- Specifically, the study follows the workflow below, with overarching goals indicated in parentheses. The process begins by moving from theory to estimands and then to design (*tutorial and model validation*). First, a CJ structure of interest is defined and its assumptions are made explicit based on the relevant literature. Next, the estimands of interest are specified and formally defined as target parameters. Finally, a synthetic conceptual population is simulated to reflect the defined structure and assumptions. -->

<!-- The workflow then proceeds from design to data (*tutorial and model validation*) by generating synthetic random samples and comparison datasets from the conceptual population. Based on this setup, models for analysing the synthetic data are specified for both the CBTL and ITCJ approaches (*tutorial and model validation*). Prior predictive checks are then conducted (*tutorial*), followed by model fitting to obtain estimates using both approaches. These estimates are evaluated through diagnostic and post-predictive assessments, including stationarity, convergence, mixing, parameter recovery, and predictive performance. Where necessary, models are iteratively refined until they meet predefined criteria for adequacy. Finally, the resulting estimates are used to recover target effects and generate predictions under both CBTL and ITCJ analyses, which are then compared in terms of interpretability and predictive accuracy (*model validation*). -->
