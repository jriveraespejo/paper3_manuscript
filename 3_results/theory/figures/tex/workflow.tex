...
\begin{tikzpicture}
    ...
    \node at (3.8,0) (g) {Diagnostics};
    \node at (4.1,-1) (h) {Post predictive};
    \node at (0.5,-1.5) (i) {Prior predictive};
    \node at (4.7,-2) (k) {Effects};
    \node at (5.1,-3) (j) {Predictions};
    ...
    \draw[-latex] (c) edge [out=345, in=180] node {} (k); % Estimand -> Effects
    \draw[-latex] (c) edge [out=335, in=180] node {} (j); % Estimand -> Predictions
    ...
    \draw[-latex, dashed] (e) edge [out=330, in=80] node {} (i); % Estimator -> Prior
    
    \draw[-latex, dashed] (f) edge [out=190, in=45] node {} (e); % Synthetic Estimates -> Estimator
    \draw[-latex, dashed] (f) edge [out=330, in=45] node {} (g); % (Synthetic) Estimates -> Diagnostics
    \draw[-latex, dashed] (f) edge [out=340, in=30] node {} (h); % (Synthetic) Estimates -> Post
    \draw[-latex] (f) edge [out=5, in=30] node {} (j); % (Synthetic) Estimates -> Effects
    \draw[-latex] (f) edge [out=355, in=30] node {} (k); % (Synthetic) Estimates -> Predictions
    
    \draw[-latex, dashed] (g) edge [out=180, in=0] node {} (e); % Diagnostics -> Estimator
    \draw[-latex, dashed] (h) edge [out=180, in=350] node {} (e); % Post -> Estimator
    \draw[-latex, dashed] (i) edge [out=140, in=260] node {} (e); % Prior -> Estimator
