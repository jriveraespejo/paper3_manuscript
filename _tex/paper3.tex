...
\begin{abstract}
The classical CJ analysis has become the standard workflow for analyzing
comparative judgment (CJ) data in education, linguistics, and
psychology. Researchers favor it because it offers a convenient and
simple method for measuring traits and conducting subsequent analyses.
Its simplicity rests on two key features: reliance on narrowly specified
Thurstone-based models to estimate latent traits, and te useof ad hoc
...
often observed in applied CJ settings. The approach enables the
development of models tailored to the assumed data-generating process of
the CJ system under study, thereby reducing reliance on simplifying
assumptions. Moreover, by integrating measurement and inference within a
single framework, it also removes dependence on ad hoc analytical
procedures. Nonetheless, these potential advantages require empirical
validation.

Thus, this study empirically validates the proposed analytical workflow
for CJ using a simulated speech-quality dataset, benchmarking its
performance against the classical CJ analysis. It also demonstrates its
practical implementation, adopting a tutorial format with step-by-step
guidance on data simulation, prior specification, model estimation, and
...
theoretical principles \citep{Thurstone_1927a, Thurstone_1927b} with key
design features of CJ assessment. This integration enables researchers
to develop models tailored to the assumed data-generating process of the
CJ system under study, reducing reliance on simplifying assumptions.
Moreover, by integrating measurement and inference within a single
framework, the ITCJ analysis also eliminates the need for ad hoc
analytical procedures.

Given the persistent challenges in education, linguistics, and
psychology--where CJ data frequently exhibit substantial heterogeneity
and design-specific variability
\citep{Thurstone_1927a, Andrich_1978, vanDaal_et_al_2016, Lesterhuis_et_al_2018, Chambers_et_al_2022}--the
flexibility of the ITCJ analysis is particularly valuable. More broadly,
any discipline implementing CJ assessments can benefit from an approach
that enables researchers to model the processes they aim to investigate,
rather than those imposed by restrictive statistical models.
...
applied to CJ data: the CCJ and ITCJ analyses. Section~\ref{sec-methods}
details the assumed data-generating process for the simulated dataset,
the simulation procedure, the practical implementation of each
analytical approach, and the evaluation criteria aligned with the
research goals. Section~\ref{sec-results} presents the data description
and modeling results. Section~\ref{sec-discussion} interprets the
findings, outlines future research directions, and considers the study
limitations. Finally, Section~\ref{sec-conclusion} offers the concluding
remarks.
...
CCJ and the ITCJ analysis. The CCJ approach relies on a sequence of
separate analytical steps to estimate traits and draw inferences. In
...
The CCJ analysis implements a sequence of separate analytical steps,
...
apply narrowly defined Thurstone-based model to the CJ data to produce
two outputs: (1) point estimates of stimulus latent scores along with
their standard errors, and (2) residuals at the comparison level. These
outputs form the basis for all subsequent analyses.

Second, researchers summarize or fit regression models to the stimulus
latent score estimates. This step serves multiple purposes, including
...
stimulus latent scores to examine whether children's age or hearing
status affects their intelligibility scores. Similarly,
\citet{Maydeu-Olivares_et_al_2005} fitted multiple factor models to
latent utility means representing preferences for compact cars to assess
whether these preferences could be explained by differences among
individuals making the comparisons.

Third, analysts summarize or fit regression models to the BTL residuals.
This step usually seeks to aggregate the remaining variability at the
judge level, partition residual variability between and within judges,
test for systematic biases, and identify potential misfitting judgments,
stimuli, or judges. For instance, \citet{Wu_2025} fitted an analysis of
variance (ANOVA) model to rater's infit statistic to examine whether
rater expertise influence judgments behavior. The infit statistics is a
weighted average of the squared Pearson residuals
\citep{Wright_et_al_1982}.

While this stepwise approach is the standard practice in fields such as
education \citep[see,][]{Wu_2025}, linguistics
\citep[see,][]{Thwaites_et_al_2024} and psychology
\citep[see,][]{Maydeu-Olivares_et_al_2005}, it presents several
limitations. First, each stage treats outputs from previous steps as
fixed data, rather than acknowledging their status as uncertain
parameter estimates. Failure to account for this uncertainty can
introduce bias and decrease the precision of inferences. The direction
and magnitude of these biases can be unpredictable: results may be
attenuated, amplified, or remain unaffected depending on the uncertainty
in the scores and the actual effects being tested
