...
\begin{abstract}
A particular data analysis workflow has become the standard approach for
analyzing comparative judgment (CJ) data, because it provides a simple
method for measuring traits and conducting statistical inferences. The
workflow's simplicity stems from two key features: (1) the use of the
Bradley-Terry-Luce (BTL) model, which imposes an extensive set of
simplifying assumptions about traits, judges, and stimuli in CJ
assessments; and (2) the use of ad hoc procedures to handle inferences,
including hypothesis testing. However, recent studies question whether
the BTL assumptions hold in contemporary CJ applications and whether the
ad hoc procedures effectively fulfill their intended analytical goals.
...
A particular data analysis workflow has become the standard approach for
analyzing CJ data \citep[see e.g.,][]{Thwaites_et_al_2024}. Researchers
favor this approach because it provides a simple method for measuring
traits and conducting statistical inferences
\citep{Andrich_1978, Pollitt_2012b}. This simplicity, in turn, arises
from two key features. First, the workflow relies on the
Bradley-Terry-Luce (BTL) model \citep{Bradley_et_al_1952, Luce_1959},
which imposes an extensive set of simplifying assumptions about traits,
judges, and stimuli in CJ assessments
\citep{Thurstone_1927b, Bramley_2008}. Second, the workflow uses ad hoc
procedures to handle inferences, including data summaries and hypothesis
testing \citep{Pollitt_2012b}.
...
\citet{Rivera_et_al_2025} argue that while the assumptions of equal
...
Furthermore, the same authors note that although ad hoc procedures
simplify data analyses, the use of untested methods can also undermine
the validity of statistical inferences derived from CJ data
...
eliminates the dependence on ad hoc hypothesis-testing procedures.
Ultimately, this approach has the potential to produce reliable trait
estimates and accurate statistical inferences. However, its
effectiveness still requires empirical validation.
