...
analyzing comparative judgment (CJ) data. Researchers favor this
approach because it provides a simple method for measuring traits and
conducting related analysis. The simplicity of the approach arises from
two key features. First, it relies on the Bradley-Terry-Luce (BTL) model
to estimate latent traits. Second, it uses ad hoc procedures to conduct
related analysis. However, recent studies question whether the BTL model
assumptions hold in contemporary CJ applications and whether the ad hoc
...
Information-Theoretical model for CJ using a simulated speech-quality
dataset, benchmarking its performance against the classical BTL
analysis. Moreover, the study demonstrates how to implement the model in
practice. Specifically, the manuscript adopts a tutorial format,
providing step-by-step guidance on data simulation, prior specification,
model estimation, and interpretation using \texttt{Stan}, \texttt{R},
and the interface packages \texttt{cmdstan} and \texttt{brms}.
Ultimately, the study equips researchers with practical tools to apply
the model to more complex CJ assessments.
\end{abstract}
...
analyses \citep{Andrich_1978, Pollitt_2012b}. The simplicity of the
approach arises from two key features. First, it relies on the
...
Second, it uses ad hoc procedures to conduct related analysis, including
data summarization and statistical inference \citep{Pollitt_2012b}.
...
Information-Theoretical model for CJ (hereafter, ITCJ analysis). The
...
estimates of stimulus latent scores along with their standard errors,
and (2) residuals at the stimulus level. These outputs form the basis
...
\citet{Maydeu-Olivares_et_al_2005} \ldots{}
...
\section{Research goals}\label{sec-introduction_goals}

It is an ``assist'' to connect the discussion of the 2nd paper with the
research goals of this: ``thi is a model that it is hard to implement''
...
interpretation using \texttt{Stan}\citep{Stan_2026a, Stan_2026b},
\texttt{R} \citep{R_2015}, and the interface packages \texttt{cmdstan}
\citep{Gabry_et_al_2025b} and \texttt{brms}
\citep{Burkner_2017, Burkner_2018}. Ultimately, the study equips
researchers with practical tools to apply the model to more complex CJ
assessments.
